...
export default function PlanPage() {
    ...
    const handleOpen = (availablePlantList: Plant[]) => {
        setOpen(true);
    ...
    let currLength: number = treeList.length + shrubList.length + canopyList.length + understoryList.length;
    ...
    return (
        <>
            ...
            <Grid container className="screen-padded">
                ...
                <Grid item xs={8}>
                    <Stack spacing={3}>
                        <SelectorBox title="Trees" selectorList={ treeList } openFunc={ handleOpen } closeFunc={ handleClose } />
                        <SelectorBox title="Shrub" selectorList={ shrubList } openFunc={ handleOpen } closeFunc={ handleClose } />
                        <SelectorBox title="Canopy" selectorList={ canopyList } openFunc={ handleOpen } closeFunc={ handleClose } />
                        <SelectorBox title="Understory" selectorList={understoryList} openFunc={ handleOpen } closeFunc={ handleClose } />
                    </Stack>
                </Grid>     
                <Grid item xs={2}>
                    <Button disabled />
